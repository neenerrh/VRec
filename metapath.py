...
def add_user_video_interaction_into_graph(positive_rating):
    ...
    for pair in positive_rating:
        ...
        Graph.add_edge(video_node, user_node)
    ...
    for user1 in users:
      for user2 in users:
         ...
         if Graph.has_node(user1) and Graph.has_node(user2):
            #print(user1+','+user2)
            mine_paths_between_nodes(Graph, user1, user2, maxLen, sample_size, fw_file)
    #for pair in rating_pair:
        #user_id = pair[0]
        #video_id = pair[1]
        #user_node =  'u'+user_id
...
if __name__ == '__main__':
    ...
    parser.add_argument('--pathlength', type=int, dest='path_length', default=2, help='length of paths with choices [3,5,7]')
    parser.add_argument('--samplesize', type=int, dest='sample_size', default=2, \
